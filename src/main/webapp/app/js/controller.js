		...
		.controller(
				...
				[
						...
						function($scope, $location, $http) {
							...
							/*
							 * var pname; var packname=[]; var cname=[[]]; var
							 * mname=[[]];
							 */

							
							//var JSONobj = {	"name" : "flare",children : [{children : [{}]}]};
							var JSONobj1 = {};
							//var semaphore = false;
							...
							function createJSON(){
								...
								.then(
										function(response) {
											if (response.data != null) {
												JSONobj1={"name":"flare1",children:[]};
												JSONobj1.children=[{name:response.data.projectName,children:[]}];
											
												$http
												.get("../packagedetailses?find=ByProjectId&projectId=" + response.data.id)
												.then(
														function(
																response1) {
															if (response1.data != null) {
																	...
																	for (var i = 0; i < response1.data.length; i++) {
																		...
																		(function(i){
																			pck.push({name:response1.data[i].packageName,children:[]});
																		     $http.get("../classdetailses?find=ByPackageId&packageId="
																					+ response1.data[i].id)
																		      .then(function (response2) {
																		         $scope.res2 = response2.data;
																		    	  cls.push($scope.res2);
																		    	  n.push(i);
																		    	  
																		    	 
																												
																												for (var j = 0; j < $scope.res2.length; j++) {
																													
																													
																													(function(j){
																														
																																																											
																														//console.log($scope.res2[i][j].id);
																													     $http.get("../methoddetails?find=ByClassId&classId="+$scope.res2[j].id)
																													      .then(function (response4) {
																													    	  //console.log(response4);
																													        $scope.res4=response4.data;
																													        mtd.push({name:$scope.res4});
																	...
																	setTimeout(function(){
																	for(var x in n){
																		for(var y=0;y<cls[x].length;y++){
																		pck[n[x]].children.push({name:cls[x][y].className,children:[]});
																		/*for(var z=0;z<mtd[y].length;z++){
																			pck[n[x]].children[y].children.push({name:mtd[y][z].methodName});
																		}*/
																	...
																	JSONobj1.children[0].children=pck;
																	alert("Tree Created");
																	createTree(JSONobj1);
																	},delay);
